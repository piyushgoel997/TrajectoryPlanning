...
def policy_fn(inp):
    ...
    act = conv_layer(norm_inp, 3, 24, 5, 2, activation_function='relu', name='conv1')
    act = conv_layer(act, 24, 36, 5, 2, activation_function='relu', name='conv2')
    act = conv_layer(act, 36, 48, 5, 2, activation_function='relu', name='conv3')

    act = conv_layer(act, 48, 64, 3, 1, activation_function='relu', name='conv4')
    act = conv_layer(act, 64, 64, 3, 1, activation_function='relu', name='conv5')

    flat = tf.layers.flatten(act)

    # TODO 3 fc layers with 100, 50, 10 neurons. But figure out the inp_size to fc1 and out_size of fc3
